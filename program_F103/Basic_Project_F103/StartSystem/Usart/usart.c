	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
            ...
            else if(temp_i == 2)OLED_ShowNum(temp_i*16,0,rece_data,0,8);
            else if(temp_i == 3){OLED_ShowNum(temp_i*16,0,rece_data,0,8);}
            temp_i++;
...
void usart1_send_char(u8 c)      //串口字节发送
{
...
void usart1_send_string(u8 *data)        //串口字符串发送
{
	while(*(data))
          usart1_send_char(*(data++));	//发送数据到串口1
}
